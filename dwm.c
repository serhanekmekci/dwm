...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > (x = selmon->ww - (int)TEXTW(stext) + lrpad - 2 * sp)) {
 			click = ClkStatusText;
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				XMoveResizeWindow(dpy, m->barwin, m->wx + sp - 2 * bb, m->by + vp, m->ww - 2 * sp, bh);
			}
			focus(NULL);
			arrange(NULL);
		}
...
togglebar(const Arg *arg)
{
	...
	updatebarpos(selmon);
	XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + sp, selmon->by + vp, selmon->ww - 2 * sp - bb, bh);
	arrange(selmon);
}
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		if (m->barwin)
			continue;
		m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by + vp, m->ww - 2 * sp - bb, bh, bb, depth,
		                          InputOutput, visual,
