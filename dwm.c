...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, iscentered, issticky;
	pid_t pid;
	Client *next;
	Client *snext;
	Client *swallowing;
...
static void setlayoutmenu(const Arg *arg);
...
bartabcalculate(
	...
	for (c = m->clients, i = 0; c; c = c->next) {
		...
		if (clientsnmaster + clientsnstack == 0 || floatlayout) {
			 x = offx + (((m->mw - offx - sw - 2*sp) / (clientsnmaster + clientsnstack + clientsnfloating)) * i);
			 w = (m->mw - offx - sw - 2*sp) / (clientsnmaster + clientsnstack + clientsnfloating);
			 tgactive = 1;
		} else if (!c->isfloating && (fulllayout || ((clientsnmaster == 0) ^ (clientsnstack == 0)))) {
			 x = offx + (((m->mw - offx - sw -2*sp) / (clientsnmaster + clientsnstack)) * i);
			 w = (m->mw - offx - sw - 2*sp) / (clientsnmaster + clientsnstack);
			 tgactive = 1;
		} else if (i < m->nmaster && !c->isfloating) {
			 /*x = offx + ((((m->mw * m->mfact) - offx) /clientsnmaster) * i);*/
			 /*w = ((m->mw * m->mfact) - offx) - 2*sp / clientsnmaster;*/
			 x = offx + (((m->mw - offx - sw -2*sp) / (clientsnmaster + clientsnstack)) * i);
			 w = (m->mw - offx - sw - 2*sp) / (clientsnmaster + clientsnstack);
			 /*tgactive = 1;*/
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		else if (ev->x > (selmon->ww - (int)TEXTW(stext) + lrpad - 2 * sp - getsystraywidth())) {
			 x = (selmon->ww - (int)TEXTW(stext) + lrpad - 2 * sp - getsystraywidth());
			 click = ClkStatusText;
...
resizebarwin(Monitor *m) {
	...
	if (showsystray && m == systraytomon(m))
		w -= getsystraywidth();
	XMoveResizeWindow(dpy, m->barwin, m->wx + sp - bb, m->by + vp, w - 2 * sp, bh);
}
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		if (showsystray && m == systraytomon(m))
			w -= getsystraywidth();
		m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by + vp, w - 2 * sp - bb, bh, bb, depth,
		                          InputOutput, visual,
